...
# Every file a version bump may REWRITE, and therefore every file the release
# commit MUST stage. This list is the other half of every `_sync_*` function: a
# file that a sync writes to disk but that is missing HERE is INERT — its change
# is never committed, the tree is left dirty after the publish, and the sync
# silently never lands. That is exactly what happened to README.md: the badge
# sync below was written (TRDD-SVEILW09) and unit-tested, yet the badge still
# drifted a release behind for months because its output was thrown away at
# commit time. Keep this list and the `_sync_*` set in lockstep.
RELEASE_STAGED_FILES: tuple[str, ...] = (
    ".claude-plugin/plugin.json",
    "pyproject.toml",
    "package.json",
    "Cargo.toml",
    # Regenerated by _sync_uv_lock(); unstaged it ships a stale lockfile and the
    # next run cuts an extra `chore: update uv.lock` housekeeping commit.
    "uv.lock",
    # Rewritten by _sync_readme_version(); see the note above.
    "README.md",
    "CHANGELOG.md",
    "cliff.toml",
    ".gitignore",
...
Examples:
  ...
  %(prog)s --patch --dry-run    # run every validation step fully, then
    ...
    staged: list[str] = []
    for name in RELEASE_STAGED_FILES:
