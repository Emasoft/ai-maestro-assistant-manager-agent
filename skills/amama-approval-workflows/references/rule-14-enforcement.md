...
| Assigning a COS changes the authority structure (MANAGER, R29) | `assign-cos` | COS has approval power over team operations. Per R29 the MANAGER creates the COS as part of team creation (the server auto-creates it on `aimaestro-teams.sh create`) -- no USER approval, no dashboard step, no sudo/governance password (R32). Re-assigning an existing team's COS to a different agent is likewise a MANAGER action via the teams CLI |
| Removing a COS removes a governance checkpoint (MANAGER, R29) | `remove-cos` | A team's COS is removed by the MANAGER as part of deleting/reconfiguring the team (R29) -- AID-authorized, no USER approval and no sudo/governance password (R32). A team is FROZEN without its COS + 5 base members (R31), so the MANAGER owns the COS lifecycle end-to-end |
