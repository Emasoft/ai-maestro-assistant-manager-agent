...
> **R42.8 IS RATIFIED — verified first-hand 2026-08-08 against `docs/GOVERNANCE-RULES.md` v5.3.2 on `governance-rules`.** Source column: `Explicit (USER — 2026-08-05, ai-maestro#125, TRDD-AODXPI5E)`. Cite it as settled governance.
>
...
>
> **Verb scope — the doc and the deployed CLI do not currently agree, and this skill follows the CLI.**
> The ratified row names **`read-prompt` and `answer` ONLY**, and explicitly excludes `inject`, `slash` and `queue` (they deliver an arbitrary command, so they express the *caller's* decision and stay self-only for every title; the server 403s them cross-agent). That exclusion is authoritative and this skill obeys it.
> But the row does **not** name **`block-state`**, while `aimaestro-session.sh help` permits it cross-agent (MANAGER: any agent; COS: own team; never an ASSISTANT). This skill uses `block-state` in DETECT for a measured reason: `read-prompt` returns what the plugin *hook* recorded, and across **419 live chat-state files it carried `AskUserQuestion` 0/419 times** — so a MANAGER obeying the doc's "ONLY" literally is blind to the single prompt shape that blocks an agent indefinitely, and sees `null` while the agent looks healthy. Until the two are reconciled, follow the deployed CLI (authoritative per the line above) and treat the doc's verb list as under-specified rather than as a prohibition on reading state. Divergence reported to the hub; do not "fix" it by dropping the DETECT step.
