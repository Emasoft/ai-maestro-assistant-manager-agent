...
- **S8.2** — This repo's value for the self-identification line `PRRD G1.2` mandates is the role name **ai-maestro-assistant-manager-agent (the MANAGER)**. This rule supplies that slot value and nothing else: the sentence itself belongs to `G1.2` and is never restated here. A rule that calls itself "the concrete instance of X" must CITE X or be deleted — restating X produces a copy that claims to be a reference, so a bump to X leaves the copy asserting the old form with nothing detecting the divergence (this rule previously mandated a different sentence than `G1.2`, undetected until 2026-08-02, `ai-maestro#109`).
