...
**R6 v3 routing constraint (2026-05-05).** Whenever the requested operation's TARGET agent is a team-internal title (ORCH, ARCH, INT, MEMBER, or any custom team-layer title), the matrix verdict applies BUT execution routes through the team's CHIEF-OF-STAFF — never a direct path. Concretely: a verdict of `approve-autonomously` for a team-internal target means "AMAMA's AMP message goes to the team's COS as a delegation request — the COS is the operation's executor — and that delegated execution is logged under the same Approval-ID". Direct AMP messaging to team-internal targets is FORBIDDEN for AMAMA under R6 v3. The only exception is HUMAN, which retains universal access. This constraint applies regardless of state (active/monitoring/away/dnd) and regardless of source-role.
